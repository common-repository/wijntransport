...
(function($) {
    ...
    var filterSidebar = $('.js-sidebar');

    //Collapsable filter section
    filterSidebar.on('click', '.js-filter-header', function() {
        $(this).parent().find('.cc__filter').slideToggle();
        $(this).toggleClass('cc--open');
    });

    //Show more
    filterSidebar.on('click', '.js-show-more', function() {
        var showMoreContainer = $(this).parent();

        $(this).addClass('cc--hide');
        showMoreContainer.find('.js-show-less').removeClass('cc--hide');

        var filterList = showMoreContainer.parent().find('.js-filter-list');
        filterList.find('.cc__filter-item').removeClass('cc--hide');
    });

    filterSidebar.on('click', '.js-show-less', function() {
        var showMoreContainer = $(this).parent();

        $(this).addClass('cc--hide');
        showMoreContainer.find('.js-show-more').removeClass('cc--hide');

        var filterList = showMoreContainer.parent().find('.js-filter-list');
        filterList.find('.cc__filter-item:nth-child(n+7)').addClass('cc--hide');
    });


    //Filters
    var updatedFilters = JSON.parse(JSON.stringify(WT.activeFilters));

    filterSidebar.on('change', '.js-filter', function() {
        var filterValue = $(this).val();
        var filterType = $(this).data('type');

        if(this.checked) {
            if (updatedFilters[filterType].indexOf(filterValue) === -1) {
                updatedFilters[filterType].push(filterValue);
            }
        } else {
            updatedFilters[filterType] = updatedFilters[filterType].filter(function(value) { return value !== filterValue; });
        }

        if (dequal(WT.activeFilters, updatedFilters)){
            filterSidebar.find('.js-filter-submit').addClass('cc--hidden');
        } else {
            filterSidebar.find('.js-filter-submit').removeClass('cc--hidden');
        }
    });

    filterSidebar.on('click','.js-filter-submit', function(e) {
        e.preventDefault();
        var url = queryString.default.stringifyUrl({ url: WT.listingPageUrl, query: updatedFilters }, { arrayFormat: 'bracket', skipEmptyString: true, skipNull: true });

        window.location.assign(url);
    });

    //Search box
    var debounceFunction = _.debounce(function(inputValue) {
        WT.activeFilters.search = inputValue;
        updatedFilters.search = inputValue;
        var url = queryString.default.stringifyUrl({ url: WT.listingPageUrl, query: updatedFilters }, { arrayFormat: 'bracket', skipEmptyString: true, skipNull: true });
        $.ajax({
            type:		'GET',
            url:		url,
            ...
            success: function(result) {
                ...
                var filtersContent = $('<div></div>').append($(result)).find('.js-filter-list-container').html();

                if (content) {
                    $('.js-search-results').html(content);
                    $('.js-filter-list-container').html(filtersContent);
    ...
    $('.js-open').on('click', function() {
        filterSidebar.removeClass('cc--closed');
        $('body').addClass('cc--filters-on');
    });

    $('.js-close').on('click', function() {
        filterSidebar.addClass('cc--closed');
        $('body').removeClass('cc--filters-on');
    });

    //Search producer filter list
    filterSidebar.on('input', '.js-filter__input-producer', function() {
        var value = this.value.toLowerCase().trim();

        if (!value) {
            $('.js-filter__list-producer li').removeAttr('style');
            $('.js-filter__more-producer').removeClass('cc--hide');
        } else {
            $('.js-filter__more-producer').addClass('cc--hide');
            $('.js-filter__list-producer li').show().filter(function () {
                return $(this).find('.cc__filter-text').text().toLowerCase().trim().indexOf(value) === -1;
            }).hide();
        }
    });
})(jQuery);
